...
export function ChatPanel({ phone, patientName, clinicId, onClose, style }: ChatPanelProps) {
  ...
  const { data: messages = [], isLoading } = useListHandoffMessages(clinicId, phone, undefined, {
    query: {
      queryKey: getListHandoffMessagesQueryKey(clinicId, phone),
      refetchInterval: 3000, // polling a cada 3s para mostrar respostas do cliente em tempo real
    },
  })
