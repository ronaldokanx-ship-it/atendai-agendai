  ...
  const filtered = React.useMemo(() => {
    ...
    return logs.filter(log => {
      if (q && !log.patientPhone.includes(q) &&
          !formatPhone(log.patientPhone).toLowerCase().includes(q) &&
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-x-4 gap-y-2 mb-4">
                      <div className="flex items-center gap-2 flex-wrap">
                        <Badge
                          ...
                          className={cn(
                            "font-mono text-xs",
                            isLidJid(log.patientPhone) && "border-yellow-400/60 text-yellow-700 dark:text-yellow-400 bg-yellow-50 dark:bg-yellow-950/30"
                          )}
                          title={isLidJid(log.patientPhone) ? `JID interno: ${log.patientPhone}` : log.patientPhone}
                        >
                          {isLidJid(log.patientPhone) ? "🔒 WhatsApp Privacy" : formatPhone(log.patientPhone)}
                        </Badge>
