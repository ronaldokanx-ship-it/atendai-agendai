...
router.post("/clinics/:clinicId/handoffs/:phone/messages", async (req, res): Promise<void> => {
  ...
  // Busca instância Evolution da clínica
  const [clinic] = await db.select({ evolutionInstanceName: clinicsTable.evolutionInstanceName }).from(clinicsTable).where(eq(clinicsTable.id, clinicId));

  // Envia pelo WhatsApp (Evolution API) se disponível
  if (clinic?.evolutionInstanceName) {
    try {
      await sendTextMessage(clinic.evolutionInstanceName, phone, content);
    } catch (err) {
      req.log.error({ err }, "[Handoff] Falha ao enviar mensagem pelo Evolution API");
    }
  }
