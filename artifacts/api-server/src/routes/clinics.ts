...
router.patch("/clinics/:id", async (req, res): Promise<void> => {
  ...
  const OWNER_ONLY_FIELDS = ["aiEnabled", "schedulingEnabled", "asaasApiKey", "mercadoPagoAccessToken", "whatsappPhoneNumberId", "whatsappAccessToken", "evolutionInstanceName", "aiExternalApiKey"];
  if (role === "supervisor") {
