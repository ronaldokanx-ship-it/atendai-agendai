...
import { clinicToChannel } from "../lib/whatsapp-provider";
import {
  sendMetaTextMessage,
  sendMetaListMessage,
...
async function sendText(clinic: { whatsappPhoneNumberId: string | null; whatsappAccessToken: string | null }, phone: string, text: string) {
  const config = { phoneNumberId: clinic.whatsappPhoneNumberId ?? "", accessToken: clinic.whatsappAccessToken ?? "" };
  if (!isMetaConfigured(config)) {
    logger.warn({ phone }, "[MetaWebhook] sendText ignorado — Meta API não configurada (falta phoneNumberId ou accessToken)");
    return;
  }
  await sendMetaTextMessage(config, phone, text);
}
...
router.post("/whatsapp/meta", async (req: Request, res: Response): Promise<void> => {
  ...
  // ── Busca clínica pelo Phone Number ID (com retry em caso de connection timeout) ──
  let clinic: typeof clinicsTable.$inferSelect | undefined;
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      ...
      if (attempt < 3 && (msg.includes("timeout") || msg.includes("terminated") || msg.includes("ECONNRESET"))) {
        logger.warn({ phoneNumberId, attempt }, "[MetaWebhook] Timeout de DB — tentando novamente...");
  ...
  // ── Mutex: ignora se clínica usa Evolution API como provedor ──────────────
  if (clinic.whatsappProvider !== "meta") {
    logger.warn({ phoneNumberId, clinicId: clinic.id, provider: clinic.whatsappProvider }, "[MetaWebhook] Clínica não está usando Meta API — webhook ignorado. Altere o Provedor Ativo em Configurações.");
    return;
  }
  ...
  for (const msg of messages) {
    await processMetaMessage(clinic, metaConfig, msg, channel);
  }
});
...
async function processMetaMessage(
  ...
  channel: ReturnType<typeof clinicToChannel>,
): Promise<void> {
  ...
  try {
    if (result.interactiveList) {
      ...
      await sendMetaListMessage(metaConfig, rawPhone, {
        ...
        logger.warn({ err }, "[MetaWebhook] Lista interativa falhou — enviando texto simples");
      ...
      if (ic.options.length <= 3) {
        await sendMetaButtonMessage(metaConfig, rawPhone, {
          ...
          logger.warn({ err }, "[MetaWebhook] Botões interativos falharam — enviando texto simples");
          await sendText(clinic, rawPhone, result.reply);
        });
      } else {
        await sendMetaListMessage(metaConfig, rawPhone, {
          ...
          logger.warn({ err }, "[MetaWebhook] Lista de opções falhou — enviando texto simples");
          await sendText(clinic, rawPhone, result.reply);
        });
      }
    } else {
    ...
    logger.error({ err, clinicId: clinic.id, rawPhone }, "[MetaWebhook] Falha crítica ao enviar resposta via Meta API");
    return;
  }
