...
async function _sfText(channel: WhatsAppChannelContext, phone: string, text: string): Promise<void> {
  const buf = captureStorage.getStore();
  if (buf) { buf.texts.push(text); return; }
  await waText(channel, phone, text);
}

async function _sfList(channel: WhatsAppChannelContext, phone: string, opts: WaListOpts): Promise<void> {
  const buf = captureStorage.getStore();
  if (buf) { buf.lastList = opts; return; }
  await waList(channel, phone, opts);
}

async function _sfButtons(channel: WhatsAppChannelContext, phone: string, opts: WaButtonOpts): Promise<void> {
  const buf = captureStorage.getStore();
  if (buf) { buf.lastButtons = opts; return; }
  await waButtons(channel, phone, opts);
}

async function _sfTyping(channel: WhatsAppChannelContext, phone: string): Promise<void> {
  if (captureStorage.getStore()) return; // pula indicador de digitação em modo de teste
  await waTyping(channel, phone);
}
...
async function showServiceSelection(
  ...
  if (services.length === 1) {
    ...
    await showProfessionalSelection(clinicId, phone, channel, getSession(clinicId, phone)!);
    return;
  }
...
async function showProfessionalSelection(
  ...
  if (professionals.length === 1) {
    ...
    await showDateSelection(clinicId, phone, channel, getSession(clinicId, phone)!);
    return;
  }
...
async function showConfirmation(
  ...
  await _sfButtons(channel, phone, {
    description: text,
    buttons: [
...
async function confirmAndBook(
  ...
  try {
    ...
    if (!isRescheduling && serviceId) {
      ...
      if (mpAccessToken && servicePrice > 0) {
        try {
          ...
          await _sfText(channel, phone, pixMsg);
    ...
    await _sfText(channel, phone, successMsg);
...
async function cancelAppointment(
  ...
  try {
    ...
    if (!appt) {
      await _sfText(channel, phone, "❌ Agendamento não encontrado ou não pertence a este número.");
      clearSchedulingSession(clinicId, phone);
      return;
    }
    ...
    clearSchedulingSession(clinicId, phone);
    await _sfText(channel, phone, "❌ Não foi possível cancelar o agendamento. Entre em contato com a clínica.");
  }
}
...
export async function startSchedulingFlow(params: {
  ...
  const { clinicId, phone, channel, clinicName, patientName } = params;
  await _sfTyping(channel, phone);
  await showServiceSelection(clinicId, phone, channel, clinicName, patientName);
}
...
export async function startManagementFlow(params: {
  ...
  const { clinicId, phone, channel } = params;
  await _sfTyping(channel, phone);
  await showAppointmentList(clinicId, phone, channel);
}
...
export async function handleSchedulingSelection(params: {
  ...
  const { clinicId, phone, channel, selectedId, clinicName } = params;
  ...
  if (selectedId.startsWith("CNC|")) {
    ...
    if (isNaN(appointmentId)) return false;
    await _sfTyping(channel, phone);
    await cancelAppointment(clinicId, phone, channel, appointmentId);
    return true;
  }
  ...
  if (selectedId.startsWith("DATE|") && (session?.state === "date_select" || session?.state === "rescheduling")) {
    ...
    await _sfTyping(channel, phone);
    await showSlotSelection(clinicId, phone, channel, getSession(clinicId, phone)!, date);
    return true;
  }
  ...
  if (selectedId.startsWith("S|")) {
    ...
    await _sfTyping(channel, phone);
    await showConfirmation(phone, channel, getSession(clinicId, phone)!);
    return true;
  }
  ...
  if (selectedId.startsWith("CNF|") && session?.state === "confirming") {
    ...
    if (answer === "yes") {
      await _sfTyping(channel, phone);
      await confirmAndBook(clinicId, phone, channel, session, clinicName);
      return true;
    }
  }
...
export async function handleSchedulingFreeText(params: {
  ...
  const { clinicId, phone, channel, message, clinicName, patientName } = params;
  const session = getSession(clinicId, phone);
  ...
  if (lower.includes("cancelar") || lower.includes("sair") || lower.includes("nao quero")) {
    ...
    if (isExitIntent || isCancelSession) {
      clearSchedulingSession(clinicId, phone);
      await _sfTyping(channel, phone);
      await _sfText(channel, phone, "Tudo bem! Cancelei o atendimento 😊 Como posso ajudar?");
      return true;
    }
    ...
    clearSchedulingSession(clinicId, phone);
    await _sfTyping(channel, phone);
    await _sfText(channel, phone, "Tudo bem! Cancelei o agendamento 😊 Como posso ajudar?");
    return true;
  }
  ...
  if (isSchedulingIntent(message)) {
    clearSchedulingSession(clinicId, phone);
    await startSchedulingFlow({ clinicId, phone, channel, clinicName, patientName });
    return true;
  }
  ...
  switch (session.state) {
    ...
    case "prof_select": {
      ...
      await showProfessionalSelection(clinicId, phone, channel, session);
      break;
    }
    case "date_select": {
      ...
      if (mentionedEntry) {
        ...
        if (matchingDate) {
          ...
          await showSlotSelection(clinicId, phone, channel, getSession(clinicId, phone)!, matchingDate);
        } else {
          const dowNames: Record<number, string> = {
            0: "domingo", 1: "segunda-feira", 2: "terça-feira", 3: "quarta-feira",
            4: "quinta-feira", 5: "sexta-feira", 6: "sábado",
          };
          await _sfText(channel, phone, `Não há horários disponíveis na ${dowNames[targetDow]} nas próximas semanas. Escolha uma das datas listadas 👇`);
          await showDateSelection(clinicId, phone, channel, session);
        }
        break;
      }
      ...
      if (numDateMatch) {
        ...
        if (matchingDate) {
          ...
          await _sfText(channel, phone, `Não há horários disponíveis para o ${label}. Escolha uma das datas listadas 👇`);
          await showDateSelection(clinicId, phone, channel, session);
        }
        break;
      }
    ...
    case "slot_select":
      if (session.selectedDate) {
        await showSlotSelection(clinicId, phone, channel, session, session.selectedDate);
      }
      break;
    case "confirming":
      await showConfirmation(phone, channel, session);
      break;
  }
