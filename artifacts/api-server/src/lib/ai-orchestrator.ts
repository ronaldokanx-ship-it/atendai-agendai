...
// Cliente fallback — Groq (latência ultra-baixa; ativado quando OpenRouter esgota rate limit)
...
// Modelos em ordem de preferência — todos suportam tool use (function calling)
 ...
 */
const MODEL_COOLDOWN_MS = 60_000; // 60 segundos
...
const AI_TOOLS: OpenAI.Chat.ChatCompletionTool[] = [
  ...
  },
  {
    ...
    function: {
      ...
      description: "Salva ou atualiza os dados do cliente/paciente (nome, anotações de interesse). Use assim que souber o nome do cliente ou quando ele mencionar interesses relevantes (ex: ansiedade, curso, emagrecimento, hypnex, fobia, agendamento etc.). Pode ser chamado múltiplas vezes — as anotações são acumuladas.",
      parameters: {
        type: "object",
        properties: {
          name: {
...
async function executeToolCall(
  ...
  try {
    ...
    if (toolName === "save_patient_info") {
      const { name, notes } = args as { name?: string; notes?: string };
      ...
      const [existing] = await db
        .select({ id: patientsTable.id, name: patientsTable.name, notes: patientsTable.notes })
        .from(patientsTable)
        .where(and(eq(patientsTable.clinicId, clinicId), eq(patientsTable.phone, patientPhone)))
        .limit(1);
      ...
      if (existing) {
        ...
        await db
          ...
          .set({ name: updatedName, notes: updatedNotes })
          .where(eq(patientsTable.id, existing.id));

        logger.info({ patientId: existing.id, name: updatedName, notes }, "Paciente atualizado via save_patient_info");
      } else {
        // Cria novo registro com o que foi informado
        await db
          .insert(patientsTable)
          .values({ clinicId, name: name || "Desconhecido", phone: patientPhone, notes: notes ?? null });

        logger.info({ patientPhone, name, notes }, "Novo paciente criado via save_patient_info");
      }
...
- Status: ${knownPatient ? `${clientLabelCap} já cadastrado — Nome: **${knownPatient.name}** (ID: ${knownPatient.id}). Não peça o nome novamente.` : `Novo ${clientLabel} — **pergunte o nome logo no início da conversa** e use \`save_patient_info\` assim que ele informar.`}${knownPatient?.notes ? `\n- Anotações anteriores: ${knownPatient.notes}` : ""}${patientHistoryText}
...
- Captura de dados do ${clientLabel}: Sempre que souber o nome do ${clientLabel}, chame \`save_patient_info\` imediatamente. Quando o ${clientLabel} mencionar interesses, chame \`save_patient_info\` com uma anotação concisa. Não avise o ${clientLabel} que está salvando — faça de forma transparente.
...
Hoje é ${todayFormatted}.`;
  ...
  const callWithRetry = async (noTools = false): Promise<{ response: ChatCompletion; usedGroq: boolean }> => {
    ...
    // Fase 1: tentar cada modelo OpenRouter — pula modelos em cooldown de 429
